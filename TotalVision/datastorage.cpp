...
void XLSStorage::SaveToFile(std::vector<TimeAnalyzer::AnalyzedProcess> analyzedProcs, const std::string& filepath)
{
	...
	for (auto analyzedProcess : analyzedProcs) {
		//LPSTR wProcessName = NULL;
		//CharToOemA(analyzedprocess.processName.c_str(), wProcessName);
		cells.Get(cellid, 0).PutValue(cellid);
...
void XLSStorage::SaveToFile(std::map<DWORD, TimeAnalyzedProcess> analyzedprocesses, const std::string& filepath) {
	Aspose::Cells::Startup();
	...
	cells.Get(0, 2).PutValue(u"Использование памяти");
	cells.Get(0, 3).PutValue(u"Использование процессора");
	cells.Get(0, 4).PutValue(u"Общее время активности");
	cells.SetColumnWidth(1, 150);
	cells.SetColumnWidth(2, 30);
	cells.SetColumnWidth(3, 30);
	...
	for (auto analyzedprocess : analyzedprocesses) {
		...
		cells.Get(cellid, 3).PutValue((double)analyzedprocess.second.processCPUPersents);
		Date activeTime = {0};
		Cell dateCell = cells.Get(cellid, 4);
		Style dateStyle = dateCell.GetStyle();
		dateStyle.SetCustom("hh:mm:ss");
		dateStyle.SetHorizontalAlignment(TextAlignmentType::Center);
		dateCell.SetStyle(dateStyle);
		dateStyle.Update();
		if (dateStyle.IsDateTime()) {
			activeTime.second = analyzedprocess.second.time % 60;
			activeTime.minute = ((analyzedprocess.second.time - activeTime.second) % (60 * 60)) / 60;
			activeTime.hour = (analyzedprocess.second.time - activeTime.minute * 60 - activeTime.second) * 60 * 60;
			dateCell.PutValue(U16String((std::to_string(activeTime.hour) + ":" +
										 std::to_string(activeTime.minute) + ":" +
										 std::to_string(activeTime.second)).c_str()));
		}
		cellid++;
	}
