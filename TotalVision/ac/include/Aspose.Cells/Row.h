...
public:
    ...
    /// </summary>
    /// <param name="index">The index(position) of the cell in the cells collection of this row.</param>
    /// <returns>The Cell object at given position.</returns>
    /// <remarks>To traverse all cells in sequence without modification,
    /// using <see cref="GetEnumerator()"/> will give better performance than using this method to get cell one by one.</remarks>
    ASPOSE_CELLS_API Cell GetCellByIndex(int32_t index);
    /// <summary>
